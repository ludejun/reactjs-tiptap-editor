...
`ai-sparkwrite-editor/bubble/ai` and place it inside your toolbar.
...
- `components.Panel` replaces the whole dialog. It receives `editor`, `options`, `selectedText`, `initialPrompt`, `apply(markdown)` and `close()`; call `generateAIText(options, request, onChunk)` from `ai-sparkwrite-editor/ai` for the transport, or your own.
