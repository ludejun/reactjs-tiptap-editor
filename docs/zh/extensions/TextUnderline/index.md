...
next:
  text: Twitter
  link: /zh/extensions/Twitter/index.md
---

# Underline

为选中文本添加下划线，或对启用该样式后输入的文本添加下划线。
...
import { RichTextProvider, TextUnderline, RichTextUnderline } from 'ai-sparkwrite-editor';
...
import { Paragraph } from '@tiptap/extension-paragraph';
import { Text } from '@tiptap/extension-text';
import { TextUnderline, RichTextProvider, RichTextUnderline } from 'ai-sparkwrite-editor/vue';
import 'ai-sparkwrite-editor/style.css';
...
点击下划线按钮切换该样式。若使用自定义按钮，可调用 `editor.chain().focus().toggleUnderline().run()`。
