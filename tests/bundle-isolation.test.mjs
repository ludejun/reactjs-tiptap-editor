...
test('all bubble entry points expose matching ESM/CJS exports and declarations', async () => {
  ...
  const cjsExports = JSON.parse(
    ...
    console.log(JSON.stringify(Object.fromEntries(JSON.parse(process.argv[1]).map(path =>
      [path, Object.keys(requirePackage('ai-sparkwrite-editor/' + path.slice(2))).sort()]))));
  `,
  ...
  for (const [path, entry] of entries) {
    const specifier = `ai-sparkwrite-editor/${path.slice(2)}`;
      ...
      plugins: [
        {
          ...
          load(id) {
            if (id === '\0virtual:isolation')
              return `import { RichTextBubbleText } from 'ai-sparkwrite-editor/${entry}'; window.bubble = RichTextBubbleText;`;
          },
        },
      ],
    });
