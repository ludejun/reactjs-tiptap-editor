 ...
 * React and Vue views share one drawing. The marks drawn here are small and
 * load with the editor; the rest (Simple Icons paths and favicons, ~25KB) sit
 * in `logos-more.ts` and arrive through `loadMoreLogos()` the first time a
 * prompt shows them. Services without a mark get a monogram on their brand
 * colour (see `EmbedService.color`).
 */
...
export const BRAND_LOGOS: Record<string, string> = {
  ...
  googlemaps: svg(
    '<defs><clipPath id="gm-pin"><path d="M12 2a7 7 0 0 0-7 7c0 5 7 13 7 13s7-8 7-13a7 7 0 0 0-7-7Z"/></clipPath></defs><g clip-path="url(#gm-pin)"><rect x="3" y="1" width="9" height="9" fill="#34A853"/><rect x="12" y="1" width="9" height="9" fill="#4285F4"/><rect x="3" y="10" width="9" height="5" fill="#FBBC04"/><rect x="12" y="10" width="9" height="5" fill="#4285F4"/><rect x="3" y="15" width="18" height="9" fill="#EA4335"/></g><circle cx="12" cy="9" r="2.6" fill="#fff"/>'
  ),
  gdocs: gdoc(
    '#4285F4',
    '#A1C2FA',
...
let more: Promise<Record<string, string>> | null = null;

/** The remaining marks, fetched once and cached; `BRAND_LOGOS` is filled in as well. */
export function loadMoreLogos() {
  more ??= import('./logos-more').then((module) => {
    Object.assign(BRAND_LOGOS, module.MORE_BRAND_LOGOS);
    return module.MORE_BRAND_LOGOS;
  });
  return more;
}
